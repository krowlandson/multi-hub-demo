...
# Azure Route Server for management Azure Route Server hub

resource "azurerm_public_ip" "hub_management_ars_route_server_pip" {
  name                = "m-r-routerserver-pip"
  location            = azurerm_resource_group.hub_management_ars.location
  resource_group_name = azurerm_resource_group.hub_management_ars.name
  allocation_method   = "Static"
  sku                 = "Standard"
}

resource "azurerm_route_server" "hub_management_ars" {
  name                             = "m-r-routerserver"
  resource_group_name              = azurerm_resource_group.hub_management_ars.name
  location                         = azurerm_resource_group.hub_management_ars.location
  sku                              = "Standard"
  public_ip_address_id             = azurerm_public_ip.hub_management_ars_route_server_pip.id
  subnet_id                        = azurerm_subnet.hub_management_ars_route_server.id
  branch_to_branch_traffic_enabled = true
}

resource "azurerm_route_server_bgp_connection" "hub_management_ars" {
  for_each = toset([
    "n-h-rras-vm001",
    "i-h-rras-vm001",
    "m-h-rras-vm001",
  ...
  name            = "r-ars-to-${element(split("-", each.value), 0)}-nva-bgpconnection"
  route_server_id = azurerm_route_server.hub_management_ars.id
  peer_asn        = 66000 + element(split(".", module.virtual_machines[each.value].private_ip_address), 2)
  peer_ip         = module.virtual_machines[each.value].private_ip_address

  depends_on = [
    module.virtual_machines,
    azurerm_virtual_network_peering.hub_management_ars_to_spoke,
    azurerm_virtual_network_peering.spoke_to_hub_management_ars,
  ]
}
